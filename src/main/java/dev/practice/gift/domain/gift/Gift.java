...
public class Gift extends AbstractEntity {
    ...
    // 빌더 패턴을 제외하고 이 메소드만 유일하게 외부에서 받은 객체값으로 엔티티의 필드를 변경한다.
    // 항상 엔티티의 필드를 외부에서 받은 객체로 업데이트할 때는 신중해야 하므로, command 객체를 잘 정의해야 하고,
    // 적절한 exception을 삽입하는 것이 좋다.  
